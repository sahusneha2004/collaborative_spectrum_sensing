...
ROOT = Path("/mnt/110e4fe8-b416-4f34-8f8b-66a64d07a44f/anjani/partial/PartialObservation-main/final/PartialObservation/Saved_Models/decouple_cnn_mod/SNRs/10mAlpha5")
SNRS = [0, -2, -4, -6, -8, -10, -12, -14, -16]  # SNRs to plot
OUTPUT = Path("/mnt/110e4fe8-b416-4f34-8f8b-66a64d07a44f/anjani/partial/PartialObservation-main/final/PartialObservation/Saved_Models/decouple_cnn_mod/SNRs/10mAlpha5/decouple_cnn_roc.png")
...
def is_energy_detection_file(path):
    return "decouple_cnn_mod" in str(path).lower()
...
def main():
    ...
    for snr in SNRS:
        ...
        if not files:
            print(f"Missing Decoupled CNN ROC for SNR {snr}; skipping.")
            continue
    ...
    if plotted == 0:
        print("No Decoupled CNN ROC CSV files found. Nothing plotted.")
        return

    plt.xlabel("Probability of False Alarm (PFA)")
    plt.ylabel("Probability of Detection (PD)")
    plt.title("Decoupled CNN ROC Curves")
