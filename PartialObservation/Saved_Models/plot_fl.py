...
ROOT = Path("/mnt/110e4fe8-b416-4f34-8f8b-66a64d07a44f/anjani/partial/PartialObservation-main/final/PartialObservation/Saved_Models/FL/SNRs/RandModTrTe/GeneratedDataset")
SNRS = [0, -2, -4, -6, -8, -10]  # SNRs to plot
OUTPUT = Path("/mnt/110e4fe8-b416-4f34-8f8b-66a64d07a44f/anjani/partial/PartialObservation-main/final/PartialObservation/Saved_Models/FL/SNRs/RandModTrTe/GeneratedDataset/fl_roc.png")
...
def main():
    ...
    for snr in SNRS:
        ...
        if not files:
            print(f"Missing FL ROC for SNR {snr}; skipping.")
            continue
    ...
    if plotted == 0:
        print("No FL ROC files found. Nothing plotted.")
        return

    plt.xlabel("Probability of False Alarm (PFA)")
    plt.ylabel("Probability of Detection (PD)")
    plt.title("Federated Learning ROC Curves")
