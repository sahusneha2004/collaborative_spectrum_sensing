...
ROOT = Path("/mnt/110e4fe8-b416-4f34-8f8b-66a64d07a44f/anjani/partial/PartialObservation-main/final/PartialObservation/Saved_Models/EnergyDetect/RandMod/GeneratedDataset")
SNRS = [0, -2, -4, -6, -8, -10, -12, -14]  # SNRs to plot
OUTPUT = Path("/mnt/110e4fe8-b416-4f34-8f8b-66a64d07a44f/anjani/partial/PartialObservation-main/final/PartialObservation/Saved_Models/EnergyDetect/RandMod/GeneratedDataset/energy_detection_roc.png")
...
def read_roc_csv(path):
    ...
    pfa = [float(row["PFA"]) for row in rows if row.get("PFA") not in ("", None)]
    pd = [float(row["PD"]) for row in rows if row.get("PD") not in ("", None)]
    return pfa, pd
...
def is_energy_detection_file(path):
    return "energydetect" in str(path).lower()
...
def main():
    ...
    for snr in SNRS:
        files = [
            p for p in list(ROOT.rglob(f"ROC_SNR{snr}.csv")) + list(ROOT.rglob(f"ROC_SNR{snr}.xlsx"))
            if is_energy_detection_file(p) and snr_from_path(p) == snr
        ]

        if not files:
            print(f"Missing EnergyDetect ROC for SNR {snr}; skipping.")
            continue

        # If there are multiple runs, use the newest result folder.
        roc_file = max(files, key=lambda p: p.stat().st_mtime)
        pfa, pd = read_roc_file(roc_file)

        if not pfa or not pd:
        ...
        print(
            ...
            f"PD range {min(pd):.4g}..{max(pd):.4g}"
        )

        plt.plot(pfa, pd, marker="o", markersize=3, linewidth=1.8, label=f"SNR {snr} dB")
        print(f"Plotted SNR {snr} from {roc_file}")
        plotted += 1
        all_values.extend(pfa)
        all_values.extend(pd)

    if plotted == 0:
        print("No EnergyDetect ROC CSV files found. Nothing plotted.")
        return

    plt.xlabel("Probability of False Alarm (PFA)")
    plt.ylabel("Probability of Detection (PD)")
    plt.title("Energy Detection ROC Curves")
